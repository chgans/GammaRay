    ...
    : QObject(parent)
{
    ...
    QTimer::singleShot(1000, this, &ObjectInspector::exportSignalBindingGraph);
}
...
#include <QThread>
#include <QDebug>
void ObjectInspector::exportSignalBindingGraph()
{
    QFile dotFile("/tmp/signals.dot");
    if (!dotFile.open(QFile::WriteOnly))
        return;

    QMutexLocker lock(Probe::objectLock());
    const QVector<QObject*> &allObjects = Probe::instance()->allQObjects();
    QHash<QObject *, int> objectIndices;
    QHash<QThread*, QVector<QObject *> > objectsByThread;
    for (int i=0; i<allObjects.count(); ++i) {
        auto object = allObjects.at(i);
        if (!Probe::instance()->isValidObject(object))
            continue;
        objectsByThread[object->thread()].append(object);
        objectIndices.insert(object, i);
    }
